...
@ComponentScan
public class MultithreadedRequestScopedBeansProcessor implements BeanDefinitionRegistryPostProcessor {
	...
	public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory) throws BeansException {
		...
		beanFactory.registerScope(multithreadedRequestScope.getClass().getSimpleName(), multithreadedRequestScope);
		...
		for (String beanName : beanFactory.getBeanDefinitionNames()) {
			...
			System.out.println("Post processing bean: " + beanName + ", scope: " + definition.getScope());

			if (MultithreadedRequestScope.NAME.equals(definition.getScope())) {
				BeanDefinitionHolder proxyHolder = ScopedProxyUtils
						.createScopedProxy(new BeanDefinitionHolder(definition, beanName), registry, false);
