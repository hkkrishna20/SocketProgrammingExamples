package name.javacode.javaee.userpref.web;
...
import name.javacode.javaee.userpref.repository.UserPreferenceRepository;

@Component
@RequiredArgsConstructor(onConstructor = @__(@Autowired))
public class UserPreferenceResourceAssembler implements ResourceAssembler<UserPreference, Resource<UserPreference>> {
	@Autowired
	...
	private UserPreference newUserPreference(String name, int serviceId, String value) {
	...
	public Resource<UserPreference> toResource(UserPreference userPreference) {
		...
		Link selfLink = linkTo(
				methodOn(CONTROLLER_CLASS).findUserPreference(userPreference.getName(), userPreference.getServiceId()))
						.withSelfRel();
		...
		Link updateLink = linkTo(methodOn(CONTROLLER_CLASS).updateUserPreference(userPreference.getName(),
				userPreference.getServiceId(), userPreference.getValue())).withRel(UPDATE_LINK);
		Link deleteLink = linkTo(methodOn(CONTROLLER_CLASS).deleteUserPreference(userPreference.getName(),
	...
	public Resource<UserPreference> findUserPreference(String name, int serviceId) {
		Optional<UserPreference> userPreference = userPreferenceRepository
				.findOne(newUserPreference(name, serviceId, null));

		return userPreference.isPresent() ? toResource(userPreference.get()) : null;
	}

	public Resource<UserPreference> saveUserPreference(String name, int serviceId, String value) {
		Optional<UserPreference> userPreference = userPreferenceRepository
