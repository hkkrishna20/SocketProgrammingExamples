package name.javacode.hadoop.logminer;

import static org.junit.Assert.assertEquals;
import static org.junit.Assert.assertFalse;
import static org.junit.Assert.assertTrue;
import name.javacode.hadoop.logminer.SkippableLogRecord;
...
public class SkippableLogRecordTest {
	...
	String line = "https-aac-allied-1 aac.alliedinsurance.com 0 155.188.183.17 - - [27/Feb/2015:07:18:04 -0600] "
		...
		+ ".NET4.0E; InfoPath.2)\"";
	SkippableLogRecord rec = new SkippableLogRecord(new Text(line));

	assertFalse(rec.isSkipped());
	assertEquals("https-aac-allied-1", rec.getJvm().toString());
	assertEquals(27, rec.getDay().get());
	assertEquals(2, rec.getMonth().get());
	assertEquals(2015, rec.getYear().get());
	assertEquals(
	...
	assertEquals(200, rec.getStatus().get());
	assertEquals(207773, rec.getSize().get());
    }
