...
package name.javacode.xml.jaxb.java2schema.xmladapter.shoppingcart;
 ...
 *  HashMap - BoundType
 */
public class AdapterPurchaseListToHashMap extends
		XmlAdapter<PurchaseList, HashMap<Integer, String>> {
	...
	public PurchaseList marshal(HashMap<Integer, String> v) {
		...
		for (Iterator<Integer> i = tMap.keySet().iterator(); i.hasNext();) {
			Integer tmpI = (Integer) i.next();
			pList.entry.add(new PartEntry(tmpI.intValue(), (String) tMap
					.get(tmpI)));
		}
		return pList;
	}
