...
public class CalcIT {
	...
	WebArchive war = ShrinkWrap
		.create(WebArchive.class, "jax-rs-programmatic-security.war")
		.addPackages(true, CalcEJB.class.getPackage())
	...
	root = client
		.target("http://localhost:8080/jax-rs-programmatic-security");
    ...
    @Test
    public void testOpenToAll() {
	path = root.path("calc/subtract").queryParam("minuend", 2)
		.queryParam("subtrahend", 1);

	Builder builder = path.request(APPLICATION_JSON_TYPE);

	Response resp = builder.get();

	assertEquals(OK.getStatusCode(), resp.getStatus());

	assertEquals(Integer.valueOf(1), resp.readEntity(int.class));
    }
}
