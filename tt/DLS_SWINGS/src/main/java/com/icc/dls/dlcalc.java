...
public class dlcalc extends JFrame implements ActionListener, TextListener, FocusListener, KeyListener,
	...
	public void hiliteon() {
		for (int i = 0; i < this.nstprow; ++i) {
			if (this.tbs[i].getX() >= Math.min(this.mprx1, this.mprx2)
					& this.tbs[i].getX() <= Math.max(this.mprx1, this.mprx2)
					& this.tbs[i].getY() >= Math.min(this.mpry1, this.mpry2)
					& this.tbs[i].getY() <= Math.max(this.mpry1, this.mpry2)) {
			...
			if (this.tc1s[i].getX() >= Math.min(this.mprx1, this.mprx2)
					& this.tc1s[i].getX() <= Math.max(this.mprx1, this.mprx2)
					& this.tc1s[i].getY() >= Math.min(this.mpry1, this.mpry2)
					& this.tc1s[i].getY() <= Math.max(this.mpry1, this.mpry2)) {
			...
			if (this.tcs[i].getX() >= Math.min(this.mprx1, this.mprx2)
					& this.tcs[i].getX() <= Math.max(this.mprx1, this.mprx2)
					& this.tcs[i].getY() >= Math.min(this.mpry1, this.mpry2)
					& this.tcs[i].getY() <= Math.max(this.mpry1, this.mpry2)) {
			...
			if (this.tds[i].getX() >= Math.min(this.mprx1, this.mprx2)
					& this.tds[i].getX() <= Math.max(this.mprx1, this.mprx2)
					& this.tds[i].getY() >= Math.min(this.mpry1, this.mpry2)
					& this.tds[i].getY() <= Math.max(this.mpry1, this.mpry2)) {
			...
			if (this.tgs[i].getX() >= Math.min(this.mprx1, this.mprx2)
					& this.tgs[i].getX() <= Math.max(this.mprx1, this.mprx2)
					& this.tgs[i].getY() >= Math.min(this.mpry1, this.mpry2)
					& this.tgs[i].getY() <= Math.max(this.mpry1, this.mpry2)) {
			...
			if (this.th1s[i].getX() >= Math.min(this.mprx1, this.mprx2)
					& this.th1s[i].getX() <= Math.max(this.mprx1, this.mprx2)
					& this.th1s[i].getY() >= Math.min(this.mpry1, this.mpry2)
					& this.th1s[i].getY() <= Math.max(this.mpry1, this.mpry2)) {
			...
			if (this.ths[i].getX() >= Math.min(this.mprx1, this.mprx2)
					& this.ths[i].getX() <= Math.max(this.mprx1, this.mprx2)
					& this.ths[i].getY() >= Math.min(this.mpry1, this.mpry2)
					& this.ths[i].getY() <= Math.max(this.mpry1, this.mpry2)) {
			...
			if (this.tis[i].getX() >= Math.min(this.mprx1, this.mprx2)
					& this.tis[i].getX() <= Math.max(this.mprx1, this.mprx2)
					& this.tis[i].getY() >= Math.min(this.mpry1, this.mpry2)
					& this.tis[i].getY() <= Math.max(this.mpry1, this.mpry2)) {
	...
	public dlcalc() {
		...
		System.out.println("this.header" + this.header);
		...
		System.out.println("this.headerB" + this.headerB);
		...
		System.out.println("this.headerR" + this.headerR);
		...
		System.out.println("this.headerBR" + this.headerBR);

		this.urlq = getClass().getResource(this.qmname); // question mark image
		this.urle = getClass().getResource(this.errname); // error image
		this.urlw = getClass().getResource(this.wrnname); // warning gif image
		...
		this.url1 = getClass().getResource(this.logofilename1); // logofilename1 icc logo
		...
		AbstractAction abstractAction = new CutAction();
	...
	public void draw0() {
		...
		System.out.println("jlabel1"+ "      ---empty");
		this.c.gridwidth = 1;
		this.c.gridx = 0;
		this.c.gridy = 1;
		this.c.weightx = 1.0D;
		...
		System.out.println("jlabel2" +"         --empty");
		this.c.gridwidth = 4;
		this.c.gridx = 1;
		this.c.gridy = 1;
		this.c.weightx = 1.0D;
		...
		System.out.println("jlabel3"+"Team 1's innings");
		...
		System.out.println("jlabel4"+ "     --empty");
		...
		System.out.println("jlabel5"+"Team 2's innings");
		...
		System.out.println("jlabel6   "+"   --empty");
	...
	public void keyPressed(KeyEvent paramKeyEvent) {
		...
		if (paramKeyEvent.getKeyCode() == 38) {
			...
			if (paramKeyEvent.getSource() == this.team1FinalScore) {
				...
				this.team1OversInnigsAtStartOfMatch
						.setCaretPosition(this.team1OversInnigsAtStartOfMatch.getText().length());
		...
		if ((((paramKeyEvent.getKeyCode() == 82) ? 1 : 0) & (this.calclte1 ? 1 : 0) & ((this.impty[60][2] == 0) ? 1 : 0)
				& (this.resfeature ? 1 : 0)) != 0) {
			int i = paramKeyEvent.getModifiersEx();
			String str = KeyEvent.getModifiersExText(i);
	...
	public void actionPerformed(final ActionEvent actionEvent) {
		System.out.println("");
		System.out.println("4765--actionPerformed");
		...
		if (actionEvent.getSource() == this.Othb) {
			final Object showInputDialog = JOptionPane.showInputDialog(null, "select match type", "other match types",
					3, this.iconq1, this.options11, this.options11[0]);
			if (showInputDialog == this.options11[this.optnum - 1]) {
				...
				while (i != 0) {
					...
					if (showInputDialog2 != null) {
						...
						try {
							...
							if (this.maxtst <= 0) {
								JOptionPane.showMessageDialog(null, "Overs must be positive", "INVALID ENTRY:", 0,
										this.icone1);
							} else if (this.maxtst > 99) {
								JOptionPane.showMessageDialog(null, "Overs/innings must be < 100", "INVALID ENTRY:", 0,
										this.icone1);
							} else {
								i = 0;
							}
						} catch (Exception ex) {
							JOptionPane.showMessageDialog(null, "Overs must be whole numbers", "INVALID ENTRY:", 0,
									this.icone1);
						}
					} else {
						i = 0;
				...
				if (n == 0) {
					int j = 1;
					while (j != 0) {
						final String showInputDialog3 = JOptionPane.showInputDialog(null, "Enter min. overs/side:",
								null, -1);
						if (showInputDialog3 != null) {
							...
							try {
								...
								if (this.minover < 0) {
									JOptionPane.showMessageDialog(null, "Overs must non-negative", "INVALID ENTRY:", 0,
											this.icone1);
								} else if (this.minover > this.maxtst) {
									JOptionPane.showMessageDialog(null,
											"Min. overs/side must not exceed overs/innings (" + this.maxtst + ")",
											"INVALID ENTRY:", 0, this.icone1);
								} else {
									j = 0;
								}
							} catch (Exception ex2) {
				...
				if (n == 0) {
					...
					this.draw1();
					this.setVisible(true);
					this.setTitle(this.PRGMname + " - match type: Custom (" + this.maxtst + " overs/innings; min. "
							+ this.minover + " overs/side)");
		...
		if (actionEvent.getSource() == this.ChngTyp) {
			...
			if (JOptionPane.showOptionDialog(null,
					"<html><center>Changing match type will reset all entry cells.<br>Do you wish to continue?",
					"Warning", -1, 2, this.iconw1, array, array[0]) == 0) {
				this.HTname = "Home Team";
				this.ATname = "Away Team";
				this.HTnameA = "Home Team";
				this.ATnameA = "Away Team";
		...
		if (actionEvent.getSource() == this.RptSum & !this.RSopen) {
			this.binntrm = false;
			final String string = JOptionPane
					.showInputDialog(null, "Enter name of competition:", null, -1, null, null, this.compname)
					.toString();
			if (string != null) {
				this.compname = string.trim();
				final String string2 = JOptionPane
						.showInputDialog(null, "Enter first-named ('home') team:", null, -1, null, null, this.HTname)
						.toString();
				if (string2 != null) {
					...
					final String string3 = JOptionPane.showInputDialog(null, "Enter second-named ('away') team:", null,
							-1, null, null, this.ATname).toString();
					if (string3 != null) {
						this.Tname2 = string3.trim();
						this.ATname = this.Tname2;
						this.Tname2A = this.Tname2;
						this.Tname2B = this.Tname2;
						final String string4 = JOptionPane
								.showInputDialog(null, "Enter venue name:", null, -1, null, null, this.venue)
								.toString();
						if (string4 != null) {
							...
							final String string5 = JOptionPane
									.showInputDialog(null, "Enter Match Date:", null, -1, null, null, this.mdate)
									.toString();
							if (string5 != null) {
								this.mdate = string5.trim();
								final Object[] array2 = { this.Tname1B, this.Tname2B };
								final Object showInputDialog4 = JOptionPane.showInputDialog(null,
										"Which team batted first?", null, 3, this.iconq1, array2, array2[this.home1]);
								if (showInputDialog4 != null) {
									...
									this.RSstr1.append("<html><body><font size=\"3\"><b>" + this.PRGMname
											+ " Match Report:</b><br>");
									this.RSstr.append(" <br>Competition: " + this.compname + "<br>");
									this.RSstr.append(this.HTname + " vs. " + this.ATname + " at " + this.venue + " on "
											+ this.mdate + "<br> <br>");
									...
									for (int l = 0; l < this.nstprow; ++l) {
										if (this.impty[l][2] == 0 & this.ddsd[l + 1] > 0.1) {
											if (this.tds[l].getText().trim().indexOf("a") == 0
													| this.tds[l].getText().trim().indexOf("A") == 0
													| (this.omax1 - this.oltot1 < this.minover - 0.01
															& l == this.lstrow1)) {
												if (this.impty[l][3] == 0) {
													if (this.dbsd[l + 1] > 1.1) {
														this.RSstr.append(this.T1name + " " + this.nc1s[l + 1] + "/"
																+ this.ncs[l + 1] + " from "
																+ this.decToodb(this.dbsd[l + 1])
																+ " overs; Innings terminated.<br>");
													} else {
														this.RSstr.append(this.T1name + " " + this.nc1s[l + 1] + "/"
																+ this.ncs[l + 1] + " from "
																+ this.decToodb(this.dbsd[l + 1])
																+ " over; Innings terminated.<br>");
													}
												} else if (this.dbsd[l + 1] > 1.1) {
													this.RSstr.append(this.T1name + " " + this.ncs[l + 1]
															+ " down from " + this.decToodb(this.dbsd[l + 1])
															+ " overs; Innings terminated.<br>");
												} else {
													this.RSstr.append(this.T1name + " " + this.ncs[l + 1]
															+ " down from " + this.decToodb(this.dbsd[l + 1])
												...
												if (this.dbsd[l + 1] > 1.1 & this.ddsd[l + 1] > 1.1) {
													this.RSstr.append(
															this.T1name + " " + this.nc1s[l + 1] + "/" + this.ncs[l + 1]
																	+ " from " + this.decToodb(this.dbsd[l + 1])
																	+ " overs; " + this.decToodb(this.ddsd[l + 1])
																	+ " overs lost/side.<br>");
												} else if (this.ddsd[l + 1] > 1.1) {
													this.RSstr.append(
															this.T1name + " " + this.nc1s[l + 1] + "/" + this.ncs[l + 1]
																	+ " from " + this.decToodb(this.dbsd[l + 1])
																	+ " over; " + this.decToodb(this.ddsd[l + 1])
																	+ " overs lost/side.<br>");
												} else if (this.dbsd[l + 1] > 1.1) {
													this.RSstr.append(this.T1name + " " + this.nc1s[l + 1] + "/"
															+ this.ncs[l + 1] + " from "
															+ this.decToodb(this.dbsd[l + 1]) + " overs; "
															+ this.decToodb(this.ddsd[l + 1]) + " over lost/side.<br>");
												} else {
													this.RSstr.append(this.T1name + " " + this.nc1s[l + 1] + "/"
															+ this.ncs[l + 1] + " from "
															+ this.decToodb(this.dbsd[l + 1]) + " over; "
															+ this.decToodb(this.ddsd[l + 1]) + " over lost/side.<br>");
												}
											} else if (this.dbsd[l + 1] > 1.1 & this.ddsd[l + 1] > 1.1) {
												this.RSstr.append(this.T1name + " " + this.ncs[l + 1] + " down from "
														+ this.decToodb(this.dbsd[l + 1]) + " overs; "
														+ this.decToodb(this.ddsd[l + 1]) + " overs lost/side.<br>");
											} else if (this.ddsd[l + 1] > 1.1) {
												this.RSstr.append(this.T1name + " " + this.ncs[l + 1] + " down from "
														+ this.decToodb(this.dbsd[l + 1]) + " over; "
														+ this.decToodb(this.ddsd[l + 1]) + " overs lost/side.<br>");
									...
									if (!this.babnd & this.omax1 - this.oltot1 > this.minover - 0.01) {
										if (Double.valueOf(this.d37.getText()) > 1.1) {
											this.RSstr.append(this.T1name + " total: " + this.nb4
													+ " from (maximum of) " + this.d37.getText() + " overs.<br>");
										} else {
											this.RSstr.append(this.T1name + " total: " + this.nb4
													+ " from (maximum of) " + this.d37.getText() + " over.<br>");
										}
									}
									...
									if (this.ng3 > this.minover - 0.01) {
										this.RSstr.append(" <br>Overs at start of 2nd innings: " + this.ng3
												+ "; Target: " + trgfuntst1 + "<br>");
										for (int n2 = 0; n2 < this.nstprow; ++n2) {
											if (this.impty[30 + n2][2] == 0
													& (this.odbTodec(this.dis[n2 + 1]) > 0.1 || this.babnda)) {
												final int trgfuntst2 = this.trgfuntst1(this.adjfct, this.lmbfct,
														n2 + 1);
												final int parfuntst1 = this.parfuntst1(this.adjfct, this.lmbfct,
														n2 + 1);
												if (n2 < this.lstrow2) {
													if (this.impty[30 + n2][3] == 0) {
														if (this.dgsd[n2 + 1] > 1.1 & this.disd[n2 + 1] > 1.1) {
															this.RSstr.append(this.T2name + " " + this.nh1s[n2 + 1]
																	+ "/" + this.nhs[n2 + 1] + " from "
																	+ this.decToodb(this.dgsd[n2 + 1])
																	+ " overs (Par Score: " + parfuntst1 + "); "
																	+ this.decToodb(this.odbTodec(this.dis[n2 + 1]))
																	+ " overs lost.  Target: " + trgfuntst2 + "<br>");
														} else if (this.disd[n2 + 1] > 1.1) {
															this.RSstr.append(this.T2name + " " + this.nh1s[n2 + 1]
																	+ "/" + this.nhs[n2 + 1] + " from "
																	+ this.decToodb(this.dgsd[n2 + 1])
																	+ " over (Par Score: " + parfuntst1 + "); "
																	+ this.decToodb(this.odbTodec(this.dis[n2 + 1]))
																	+ " overs lost.  Target: " + trgfuntst2 + "<br>");
															...
															this.RSstr.append(this.T2name + " " + this.nh1s[n2 + 1]
																	...
																	+ " overs (Par Score: " + parfuntst1 + "); "
																	+ this.decToodb(this.odbTodec(this.dis[n2 + 1]))
																	+ " over lost.  Target: " + trgfuntst2 + "<br>");
														} else {
															this.RSstr.append(this.T2name + " " + this.nh1s[n2 + 1]
																	+ "/" + this.nhs[n2 + 1] + " from "
																	+ this.decToodb(this.dgsd[n2 + 1])
														...
														this.RSstr.append(this.T2name + " " + this.nhs[n2 + 1]
																+ " down from " + this.decToodb(this.dgsd[n2 + 1])
																+ " overs (Par Score: " + parfuntst1 + "); "
																+ this.decToodb(this.odbTodec(this.dis[n2 + 1]))
																+ " overs lost.  Target: " + trgfuntst2 + "<br>");
													} else if (this.disd[n2 + 1] > 1.1) {
													...
													if (this.dgsd[n2 + 1] > 1.1) {
														if (this.rsltcde > 0) {
															this.RSstr.append(this.T2name + " " + this.nh1s[n2 + 1]
																	+ "/" + this.nhs[n2 + 1] + " from "
																	+ this.decToodb(this.dgsd[n2 + 1])
																	+ " overs; Innings terminated (" + this.rsltdes
																			.substring(this.rsltdes.indexOf(";") + 2)
																	+ ").<br>");
														} else {
															this.RSstr.append(this.T2name + " " + this.nh1s[n2 + 1]
																	+ "/" + this.nhs[n2 + 1] + " from "
														...
														if (this.rsltcde > 0) {
															this.RSstr.append(this.T2name + " " + this.nh1s[n2 + 1]
																	+ "/" + this.nhs[n2 + 1] + " from "
																	+ this.decToodb(this.dgsd[n2 + 1])
																	+ " over; Innings terminated (" + this.rsltdes
																			.substring(this.rsltdes.indexOf(";") + 2)
																	+ ").<br>");
														} else {
															this.RSstr.append(this.T2name + " " + this.nh1s[n2 + 1]
													...
													this.RSstr
															.append(this.T2name + " " + this.nhs[n2 + 1] + " down from "
																	+ this.decToodb(this.dgsd[n2 + 1])
																	+ " overs; Innings terminated (" + this.rsltdes
																			.substring(this.rsltdes.indexOf(";") + 2)
																	+ ").<br>");
										...
										if ((this.impty[30 + this.lstrow2][2] == 1
												|| this.odbTodec(this.dis[this.lstrow2 + 1]) < 0.01)
												& this.impty[30 + this.lstrow2][1] == 0 & !this.babnda) {
											if (this.impty[30 + this.lstrow2][3] == 0) {
												if (this.dgsd[this.lstrow2 + 1] > 1.1) {
													this.RSstr.append(this.T2name + " " + this.nh1s[this.lstrow2 + 1]
															+ "/" + this.nhs[this.lstrow2 + 1] + " from "
															+ this.decToodb(this.dgsd[this.lstrow2 + 1])
															+ " overs.<br>");
												} else {
													this.RSstr.append(this.T2name + " " + this.nh1s[this.lstrow2 + 1]
															+ "/" + this.nhs[this.lstrow2 + 1] + " from "
												...
												this.RSstr.append(this.T2name + " " + this.nhs[this.lstrow2 + 1]
														+ " down from " + this.decToodb(this.dgsd[this.lstrow2 + 1])
														+ " overs.<br>");
											} else {
												this.RSstr.append(this.T2name + " " + this.nhs[this.lstrow2 + 1]
														+ " down from " + this.decToodb(this.dgsd[this.lstrow2 + 1])
									...
									if (this.rsltcde == 0) {
										this.RSstr.append(" <br>" + this.rsltdes.substring(0, this.rsltdes.indexOf(";"))
												+ "<br>");
									} else if (this.rsltcde == 1) {
										if (this.binntrm) {
											this.RSstr.append(" <br>RESULT: " + this.T1name + " "
													+ this.rsltdes.substring(0, this.rsltdes.indexOf(";")) + "<br>");
										} else {
											this.RSstr.append(" <br>RESULT: " + this.T1name + " "
													+ this.rsltdes.substring(0, this.rsltdes.indexOf(";")) + "<br>");
										}
										...
										if (this.binntrm) {
											this.RSstr.append(" <br>RESULT: " + this.T2name + " "
													+ this.rsltdes.substring(0, this.rsltdes.indexOf(";")) + "<br>");
										} else {
											this.RSstr.append(
													" <br>RESULT: " + this.T2name + " " + this.rsltdes + "<br>");
										}
									}
									this.RSstr1.append(this.RSstr.toString());
									this.RSstr.append("</html>");
									if (this.RSfrst) {
										(this.RSfrm = new mkeRS(this.dirnmeRS)).drawTable(this.RSstr, this.RSstr1,
												this.iconw1);
										...
										this.RSfrm.setBounds(0, 0, this.RSfrm.getWidth() + 20,
												this.RSfrm.getHeight() + 20);
										this.RSfrm.validate();
		...
		if (actionEvent.getSource() == this.Otble & !this.otblopen) {
			if (this.otblfrst) {
				(this.sotblfrm = new JFrame("Processing table of over-by-over Par Scores . . ."))
						.setLocationRelativeTo(this.a4);
			...
			(this.gcde = new StringBuilder("TableID" + this.PRGMno + ": "))
					.append(this.team1OversInnigsAtStartOfMatch.getText().trim());
			this.gcde.append("-");
			this.gcde.append(this.team1FinalScore.getText().trim());
			...
			this.otblstr.append(
					"<html><body><table border=\"1\" cellpadding=\"0\" cellspacing=\"0\"><tr><td rowspan=\"2\" width=\"70\">");
			this.otblstr.append(
					" <center><font size=\"3\"><b>overs<br>bowled</b></font></center></td><td rowspan=\"2\" width=\"70\"><center>");
			this.otblstr.append(
					" <font size=\"3\"><b>overs<br>remaining</b></font></center></td><td colspan=\"10\"><center><font size=\"3\">");
			this.otblstr.append(
					" <b>wickets down</b></font></center></td></tr><tr><td width=\"45\"><center><font size=\"3\"><b>0</b></font></center>");
			this.otblstr.append(
					" </td><td width=\"45\"><center><font size=\"3\"><b>1</b></font></center></td><td width=\"45\"><center><font size=\"3\">");
			this.otblstr.append(
					" <b>2</b></font></center></td><td width=\"45\"><center><font size=\"3\"><b>3</b></font></center></td><td width=\"45\">");
			this.otblstr.append(
					" <center><font size=\"3\"><b>4</b></font></center></td><td width=\"45\"><center><font size=\"3\"><b>5</b></font>");
			this.otblstr.append(
					" </center></td><td width=\"45\"><center><font size=\"3\"><b>6</b></font></center></td><td width=\"45\"><center>");
			this.otblstr.append(
					" <font size=\"3\"><b>7</b></font></center></td><td width=\"45\"><center><font size=\"3\"><b>8</b></font></center>");
			...
			this.otblstr1.append(
					"<html><body><center><font size=\"3\"><b>Table of over-by-over Par Scores</b></font></center>");
			...
			this.otblstr.append(
					"<tr><td colspan=\"12\"><table border=\"0\" cellpadding=\"0\" cellspacing=\"0\" width=\"100%\"><tr><td><font size=\"1\"><b>");
			...
			if (this.otblfrst) {
				(this.otblfrm = new mkeOtble(this.dirnme, this.otblstrT)).drawTable(this.otblstr, this.otblstr1,
						this.iconw1);
				this.otblfrst = false;
				...
				final int min = Math.min(this.otblfrm.getHeight(),
						GraphicsEnvironment.getLocalGraphicsEnvironment().getMaximumWindowBounds().height - 72);
				int n7 = 0;
				if (this.btblopen) {
					n7 += 18;
				}
		...
		if (actionEvent.getSource() == this.Btble & !this.btblopen) {
			if (this.btblfrst) {
				(this.sbtblfrm = new JFrame("Processing table of ball-by-ball Par Scores . . ."))
						.setLocationRelativeTo(this.a4);
			...
			this.btblstr.append(
					"<html><body><table border=\"1\" cellpadding=\"0\" cellspacing=\"0\"><tr><td rowspan=\"2\" width=\"70\">");
			this.btblstr.append(
					" <center><font size=\"3\"><b>overs.balls<br>bowled</b></font></center></td><td rowspan=\"2\" width=\"70\"><center>");
			this.btblstr.append(
					" <font size=\"3\"><b>overs<br>remaining</b></font></center></td><td colspan=\"10\"><center><font size=\"3\">");
			this.btblstr.append(
					" <b>wickets down</b></font></center></td></tr><tr><td width=\"45\"><center><font size=\"3\"><b>0</b></font></center>");
			...
			this.btblstr1.append(
					"<html><body><center><font size=\"3\"><b>Table of ball-by-ball Par Scores</b></font></center>");
			...
			for (int n10 = 0; n10 < this.nb3 * 6 + 1; ++n10) {
				...
				if (this.tmp <= this.olft2) {
					...
					if (this.idrwcnt == 56) {
						...
						this.btblstr1.append(
								"</b></font></td></tr></table></center><center><font size=\"3\"><b>Table of ball-by-ball Par Scores</b></font></center>");
						this.btblstr1.append((CharSequence) this.headerB);
			...
			this.btblstr.append(
					"<center>&copy; " + simpleDateFormat2.format(date2) + " International Cricket Council</center>");
			...
			if (this.btblfrst) {
				(this.btblfrm = new mkeBtble(this.dirnmeb, this.btblstrT)).drawTable(this.btblstr, this.btblstr1,
						this.iconw1);
				this.btblfrst = false;
				...
				final int min2 = Math.min(this.btblfrm.getHeight(),
						GraphicsEnvironment.getLocalGraphicsEnvironment().getMaximumWindowBounds().height - 72);
				int n12 = 0;
				if (this.otblopen) {
					n12 += 18;
			...
			try {
				...
				while (n13 == 0) {
					...
					if (fileChooser.showDialog(null, "Save") == 0) {
						...
						if (selectedFile.exists()) {
							if (JOptionPane.showConfirmDialog(null, "Overwrite existing file?", null, 0, 2,
									this.iconw1) == 0) {
								n13 = 1;
							} else {
								n13 = 0;
				...
				if (!b) {
					...
					if (description.indexOf("html") > -1) {
						...
						printStream.println("        (c) " + simpleDateFormat3.format(date3)
...
//                SwingUtilities.invokeLater((Runnable)new dlcalc.dlcalc$7(this));
		...
		} else if ((!this.bruns & this.iT2trg > -1) && (this.tis[this.iT2trg].getText().trim().indexOf("?") == 0
				& textEvent.getSource() != this.tis[this.iT2trg])) {
			this.tis[this.iT2trg].setText("");
		...
		try {
			...
			for (int iabnda = 0; iabnda < this.nstprow; ++iabnda) {
				...
				if (this.ctmp.length() > 0 & !this.babnd) {
					if (this.ctmp.length() == 1 & this.ctmp.indexOf(".") == 0) {
						...
						this.tbs[iabnda].setText(this.ctmp1.substring(0, this.ctmp1.indexOf(".")) + "0"
								+ this.ctmp1.substring(this.ctmp1.indexOf(".")));
				...
				if (this.ctmp.length() > 0 & !this.babnd) {
					if (this.ctmp.length() == 1 & this.ctmp.indexOf(".") == 0) {
						...
						this.tds[iabnda].setText(this.ctmp1.substring(0, this.ctmp1.indexOf(".")) + "0"
								+ this.ctmp1.substring(this.ctmp1.indexOf(".")));
				...
				if (this.ctmp.length() > 0 & !this.bT2trg1 & !this.babnda & !this.bT2trg) {
					if (this.ctmp.length() == 1 & this.ctmp.indexOf(".") == 0) {
						...
						this.tgs[iabnda].setText(this.ctmp1.substring(0, this.ctmp1.indexOf(".")) + "0"
								+ this.ctmp1.substring(this.ctmp1.indexOf(".")));
				...
				if (this.ctmp.length() > 0 & !this.bT2trg1 & !this.babnda & !this.bT2trg) {
					if (this.ctmp.length() == 1 & this.ctmp.indexOf(".") == 0) {
						...
						this.tis[iabnda].setText(this.ctmp1.substring(0, this.ctmp1.indexOf(".")) + "0"
								+ this.ctmp1.substring(this.ctmp1.indexOf(".")));
			...
			if (!this.bbasic2) {
				...
				if (this.oltot2 + this.oplyd2 > this.omax2 + 0.01) {
					...
					this.bovrm2a = false;
					if (textEvent.getSource() != this.g3
							& textEvent.getSource() != this.team1OversInnigsAtStartOfMatch) {
						this.bovrm2a = true;
						this.bovrm2 = false;
						this.f41.setText("ERROR: ");
						this.f41.setOpaque(true);
			...
			if (this.calclte) {
				...
				if ((this.chkallbut1 & this.calclte1 & textEvent.getSource() == this.tis[this.iPtrg])
						&& (this.tis[this.iPtrg].getText().trim().length() == 0 & this.wckt2 < 10 & !this.bT2beforeT1
								& this.impty[30 + this.iPtrg][0] + this.impty[30 + this.iPtrg][1] == 0)
						&& (this.omax2 - this.oltot2 - this.oplyd2 > 0.01 & !this.bslctd)) {
					this.defghi42.setText(
							"<html><b>NOTE:</b> Enter '<b>a</b>' to abandon innings or '<b>?</b>' for table of possible Targets");
					this.defghi42.setVisible(true);
					this.defghi42.setBackground(dlcalc.orng1);
				}
				if ((this.chkallbut1a & textEvent.getSource() == this.tds[this.iPtrga])
						&& (this.tds[this.iPtrga].getText().trim().length() == 0 & this.wckt1 < 10
								& this.impty[this.iPtrg][0] + this.impty[this.iPtrg][1] == 0)
						&& (this.omax1 - this.oltot1 - this.oplyd1 > 0.01 & !this.bslctd)) {
					this.defghi42.setText("<html><b>NOTE:</b> Enter '<b>a</b>' to abandon innings");
					this.defghi42.setVisible(true);
					this.defghi42.setBackground(dlcalc.orng1);
				}
			}
			if (textEvent.getSource() == this.g3 & this.calclte1 & this.impty[60][0] + this.impty[60][2] == 0
					& this.g3.getText().trim().length() == 0 & !this.bT2inp & !this.bslctd) {
			...
			if (this.d37.getText().trim().length() > 0 || this.wckt1 == 10) {
				...
				if ((Math.abs(odbTodec - this.oplyd1) < 0.01
						& textEvent.getSource() != this.team1OversInnigsAtStartOfMatch) || this.wckt1 == 10) {
					int n9 = 0;
					...
					if (!this.usrrn1 & textEvent.getSource() != this.team1FinalScore) {
						if ((this.tc1s[n9].getText().trim().length() > 0 & this.wckt1 < 10)
								|| (this.tc1s[n11].getText().trim().length() > 0 & this.wckt1 == 10)) {
							this.team1FinalScore.setText(new Integer(this.runs1).toString());
							this.bT2beforeT1 = false;
							this.nb4 = this.runs1;
							this.impty[60][2] = 0;
							if (this.calclte2 & this.impty[60][0] + this.impty[60][1] == 0
									& (this.calclte1 || this.chkallbut1a)) {
								this.calclte = true;
							}
						} else {
						...
						this.bincon1 = true;
							...
							& this.impty[n9][3] == 0)
							&& (this.calclte2 & this.impty[60][0] + this.impty[60][1] == 0
									& (this.calclte1 || this.chkallbut1a))) {
						this.calclte = true;
					}
				...
				if (this.lstrow2 > -1) {
					...
					for (int n21 = 0; n21 < this.lstrow2 + 1; ++n21) {
						if (this.impty[30 + n21][3] == 0) {
							...
							if (this.nh1s[n21 + 1] >= this.trgttst1
									& (n21 < this.lstrow2 || this.wckt2 == 10 || this.disd[n21 + 1] > 0.01)) {
								this.bincon = true;
								this.calclte = false;
				...
				if (this.bT2trg || this.bT2trg1) {
					...
					if (n34 > 0 || this.brslt) {
						...
						final JLabel label = new JLabel(
								"<html><body><font size=\"5\"><b><center>Table of possible Targets</center></b></font></html>",
								0);
						if (!this.bT2open) {
							...
							(this.T2trgf = new JFrame("Processing table of possible Targets . . ."))
									.setLocationRelativeTo(this.las[0]);
						...
						this.T2trgb
								.append("<html><body><table border=\"1\" cellpadding=\"0\" cellspacing=\"0\"><tr><td>");
						this.T2trgb.append(
								" <center><font size=\"3\"><b>overs&nbsp;lost&nbsp;in<br>&nbsp;current&nbsp;stoppage&nbsp;</b></font></center></td><td><center>");
						this.T2trgb.append(
								" <font size=\"3\"><b>total&nbsp;overs<br>&nbsp;in&nbsp;innings&nbsp;</b></font></center></td><td><center><font size=\"3\"><b>overs<br>&nbsp;remaining&nbsp;</b></font></center></td><td>");
						this.T2trgb.append(
								"<table border=\"0\" cellpadding=\"0\" cellspacing=\"0\"><tr><td width=45><font size=\"3\"><b><br>&nbsp;Target&nbsp;</b></font></td><td></td></table>");
						...
						this.T2trgbp
								.append("<html><body><center><table border=\"0\" cellpadding=\"0\" cellspacing=\"0\">");
						this.T2trgbp.append(
								" <tr><td colspan=\"5\"><center><font size=\"3\"><b>Table of possible Targets</b></font></center><hr></td></tr><tr><td>");
						this.T2trgbp.append(
								" <center><font size=\"3\"><b>overs&nbsp;lost&nbsp;in<br>&nbsp;current&nbsp;stoppage&nbsp;<hr></b></font></center></td><td><center>");
						this.T2trgbp.append(
								" <font size=\"3\"><b>total&nbsp;overs<br>&nbsp;in&nbsp;innings&nbsp;<hr></b></font></center></td><td><center><font size=\"3\"><b>overs<br>&nbsp;remaining&nbsp;<hr></b></font></center></td><td><center><font size=\"3\">");
						this.T2trgbp.append(
								" <b><br>&nbsp;Target&nbsp;<hr></b></font></center></td><td><font size=\"3\"><br>&nbsp;<hr></font></td></tr>");
						this.idrwcnt = 0;
						this.lmbfct = this.lmbfun();
						this.adjfct = this.adjfun(this.lmbfct);
						this.btrgachd = false;
						for (int n38 = 0; n38 < (int) Math.ceil(this.olft2) + 1; ++n38) {
							...
							if (n38 < (int) Math.ceil(this.olft2) & !this.btrgachd) {
								...
								if (this.tmpa - n38 < this.minover) {
									this.T2trgb.append(
											"<table border=\"0\" cellpadding=\"0\" cellspacing=\"0\"><tr><td width=45><center><font size=\"2\">NR</font></center></td><td></td></table>");
									this.T2trgb.append("</font></td>");
									this.btrgachd = true;
								} else if (this.impty[30 + this.iT2trg][3] == 0 & this.iT2trg >= 0) {
									if (this.nh1s[this.iT2trg + 1] >= n39) {
										this.T2trgb.append(
												"<table border=\"0\" cellpadding=\"0\" cellspacing=\"0\"><tr><td width=45><center><font size=\"2\">"
														+ n39.toString()
														+ "</font></center></td><td><font size=\"2\">(already achieved)</font></td></table>");
										this.T2trgb.append("</font></td>");
										this.btrgachd = true;
									} else {
										this.T2trgb.append(
												"<table border=\"0\" cellpadding=\"0\" cellspacing=\"0\"><tr><td width=45><center><font size=\"2\">"
														+ n39.toString() + "</font></center></td><td></td></table>");
										this.T2trgb.append("</font></td>");
									}
								} else {
									this.T2trgb.append(
								...
								if (this.tmpa - n38 < this.minover) {
									...
									if (this.nh1s[this.iT2trg + 1] >= n39) {
										...
										this.T2trgbp.append(
												"</font></center></td><td><font size=\"2\">(already achieved)</font></td>");
						...
						this.T2trgb.append(
								"<tr><td colspan=\"5\"><table border=\"0\" cellpadding=\"0\" cellspacing=\"0\" width=\"100%\"><tr><td><font size=\"1\"><b>");
						...
						this.T2trgb.append("<center>&copy; " + simpleDateFormat.format(date)
								+ " International Cricket Council</center>");
						this.T2trgb.append("</font></body></html>");
						this.T2trgbp.append(
								"<tr><td colspan=\"5\"><hr><table border=\"0\" cellpadding=\"0\" cellspacing=\"0\" width=\"100%\"><tr><td><font size=\"1\"><b>");
						...
						if (this.tmpa - this.olft2 < this.minover) {
							this.T2trgbp.append(
									"<tr><td colspan=\"5\"><font size=\"1\">NR = No Result, available overs &lt; "
											+ this.minover + ".</font></td></tr>");
							this.T2trgbpT.append("NR = No Result, available overs < " + this.minover + ".<br>");
						}
						this.T2trgbp.append("</table></center><font size=\"1\">");
						this.T2trgbp.append("<center>&copy; " + simpleDateFormat.format(date)
								+ " International Cricket Council</center>");
						...
						if (!this.bT2open) {
							...
							final int min = Math.min(this.T2trgf.getHeight(),
							...
							final int min2 = Math.min(this.T2trgf.getHeight(),
									GraphicsEnvironment.getLocalGraphicsEnvironment().getMaximumWindowBounds().height
											- 72);
							int n41 = 0;
							if (this.otblopen) {
					...
					if (this.brunsall & !this.brunord2) {
						...
						if (this.nh1s[this.lstrow2 + 1] >= this.trgt) {
							final int n43 = this.nh1s[this.lstrow2 + 1]
									- this.trgfuntst1(this.adjfct, this.lmbfct, this.lstrow2);
							this.winmarg = this.nh1s[this.lstrow2 + 1] - this.curpar1;
							if (n43 >= 0) {
								final int n44 = 10 - this.wckt2;
								if (!this.btoofew) {
									if (n44 == 1) {
										this.k3.setText("<html>&nbsp; <span style=\"color:white\">TEAM 2</span> win by "
												+ n44 + " wicket");
										this.rsltcde = 2;
									...
									if (this.omax2 - this.oplyd2 - this.oltot2 < 0.01
											& this.disd[this.lstrow2 + 1] > 0.01) {
										this.g4.setText(" ");
									} else {
										this.g4.setText("" + this.trgt);
										this.g4.setForeground(Color.red);
									}
								} else if (textEvent.getSource() != this.team1OversInnigsAtStartOfMatch
										& textEvent.getSource() != this.g3) {
									this.k3.setText("<html>&nbsp available overs &lt; " + this.minover);
									this.RptSum.setEnabled(true);
									this.k3.setBackground(dlcalc.ornge);
							...
							if (this.olft2 > 0.0 & this.wckt2 < 10) {
								if (!this.btoofew) {
									if (this.dis[this.lstrow2 + 1] > 0.01) {
										this.k3.setText("<html><font size=\"2\">&nbsp;</font>Revised Target: "
												+ this.g4.getText() + " <font size=\"3\">(" + this.i37.getText()
												+ " overs)</font>");
										this.k3.setBackground(dlcalc.blu1);
										if (this.olft2 > 1.0) {
											if (this.rtoget > 1) {
												this.k4.setText("<html>&nbsp; (" + this.rtoget + " further runs in "
														+ this.decToodb(this.olft2) + " overs)");
											} else {
												this.k4.setText("<html>&nbsp; T2 need " + this.rtoget
														+ " more run <font size=\"2\">(" + this.decToodb(this.olft2)
														+ " overs)</font>");
											}
										} else if (this.rtoget > 1) {
											this.k4.setText("<html>&nbsp; T2 need " + this.rtoget
													+ " more runs <font size=\"2\">(" + this.decToodb(this.olft2)
													+ " over)</font>");
										} else {
											this.k4.setText("<html>&nbsp; T2 need " + this.rtoget
													+ " more run <font size=\"2\">(" + this.decToodb(this.olft2)
								...
								if (!this.btoofew) {
									if (this.rtoget1 > 1) {
										this.k3.setText("<html>&nbsp; <span style=\"color:white\">TEAM 1</span> win by "
												+ this.rtoget1 + " runs");
										this.rsltcde = 1;
				...
				if (this.tmpa < this.minover & this.g3.getText().trim().indexOf("?") != 0 & this.wckt2 < 10) {
					...
					if (this.nh1s[this.iPtrg + 1] > n46) {
						...
						if (this.wckt2 < 10 & this.omax2 - this.oltot2 - this.oplyd2 > 0.01) {
							...
							if (n49 > 1) {
								this.k3.setText("<html>&nbsp; <span style=\"color:white\">TEAM 1</span> win by " + n49
										+ " runs");
								this.rsltcde = 1;
	...
	public void focusGained(final FocusEvent focusEvent) {
		...
		if ((this.chkallbut1 & this.calclte1 & focusEvent.getSource() == this.tis[this.iPtrg]
				& this.tis[this.iPtrg].getText().trim().length() == 0 & this.wckt2 < 10 & !this.bT2beforeT1
				& !this.brslt & !this.bslctd) && this.omax2 - this.oltot2 - this.oplyd2 > 0.01) {
			this.defghi42.setText(
					"<html><b>NOTE:</b> Enter '<b>a</b>' to abandon innings or '<b>?</b>' to get table of possible Targets");
			this.defghi42.setVisible(true);
			this.defghi42.setBackground(dlcalc.orng1);
		}
		...
		if ((this.iT2trg > -1 & !this.calclte) && (focusEvent.getOppositeComponent() == this.tis[this.iT2trg]
				& this.tis[this.iT2trg].getText().trim().indexOf("?") == 0)) {
			this.tis[this.iT2trg].setText("");
		}
	}
	...
	public void focusLost(final FocusEvent focusEvent) {
		...
		if (focusEvent.getSource() == this.team1FinalScore) {
			...
			if (this.team1FinalScore.getText().trim().length() == 0) {
				...
				if (this.calclte1 || this.chkallbut1a) {
					...
					if (Math.abs(odbTodec - this.oplyd1) < 0.01
							&& this.tc1s[this.iPtrga].getText().trim().length() > 0) {
						this.team1FinalScore.setText(new Integer(this.runs1).toString());
					}
				}
			}
		...
		if ((focusEvent.getSource() == this.team1OversInnigsAtStartOfMatch
				|| (focusEvent.getSource() == this.g3 & this.g3.getText().trim().indexOf("?") != 0)) & this.btoofew) {
			this.k3.setText("<html>&nbsp; available overs &lt; " + this.minover);
			this.rsltcde = 0;
			this.rsltdes = "NO RESULT; Par Score: ";
			this.RptSum.setEnabled(true);
